...
class dummy_uav(object):
    ...
    def phi_avoid_collision(self):
        ...
        vel = np.linalg.norm(np.array(self.vel_euclid.__getstate__()[1:]))
        ...
        for nbnm in self.neighbour_names:
            ...
            def distance(ind):
                ...
                d2 = np.sqrt((ind[0] - f2x) ** 2. + (ind[1] - f2y) ** 2. + (ind[2] - f2z) ** 2.)
                return d1 + d2
    ...
    def callback_pose(self, msg):
        ...
        try:
            ...
            rospy.logerr("{} KeyError!! name={} msg={}".format(self.tag, self.name, msg))
    ...
    def fly(self):
        ...
        if self._dim == 2:
            rospy.logdebug(
                "{}:{}->{} grad={} dx={}\ndfdxr[{},{}]={} dfdxn[{},{}]={} dfdx[{},{}]={}\ndfdyr[{},{}]={} "
                "dfdyn[{},{}]={} dfdy[{},{}]={}".format(self.tag, old_pos[:], goal.__getstate__()[:], grad, dx,
        ...
        if self._dim == 3:
            rospy.logdebug("{}:{}->{} grad={} dx={}\ndfdxr[{},{},{}]={} dfdxn[{},{},{}]={} dfdx[{},{},{}]={}\n"
                ...
                self.tag, old_pos[:], goal.__getstate__()[:], grad, dx,
    ...
    def take_off(self, start_at):
        ...
        # rospy.Subscriber(name='/{}/{}/geo_location'.format(vendor, self._name), callback=self.callback_geo_location,
        #                  data_class=geo_location)
        # rospy.Subscriber(name='/{}/{}/vel_euclid'.format(vendor, self._name), callback=todo,
        ...
        #                      data_class=temperature)
        #     rospy.Subscriber(name='/{}/{}/geo_location'.format(vendor, from_uav), callback=self.callback_geo_location,
        #                      data_class=geo_location)
